...
if (Transition.fold) TRANSITIONS.push({ label: 'Fold', transition: Transition.fold });
if (Transition.blur) TRANSITIONS.push({ label: 'Blur', transition: Transition.blur });
if (Transition.dotted) TRANSITIONS.push({ label: 'Dotted', transition: Transition.dotted });
...
const DISABLEDS = [{ label: 'Yes', disabled: true }, { label: 'No', disabled: false }];
...
class ExplorerComponentStore {
  constructor({ label, shape, content, size, position, transition, duration, easing, disabled }) {
    this._label = label;
...
export default ExplorerComponentStore;
